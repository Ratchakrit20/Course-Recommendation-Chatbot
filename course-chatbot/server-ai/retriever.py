...
# from config import CHROMA_ROOT
...
#         # กรณีเก็บเป็น JSON string เช่น '["a","b"]'
...
#         raise FileNotFoundError(f"Department DB not found: {persist_dir}")
...
# def search_department(department: str, query: str, k: int = 5) -> List[Dict[str, Any]]:
#     db = load_department_db(department)

#     try:
#         docs_with_scores = db.similarity_search_with_relevance_scores(query, k=k)
#         results = [
#             _normalize_result(doc, score)
#             for doc, score in docs_with_scores
#         ]
#     except Exception:
#         # fallback เผื่อบางเวอร์ชันไม่รองรับ relevance score
#         docs = db.similarity_search(query, k=k)
#         results = [
#             _normalize_result(doc, 0.0)
#             for doc in docs
#         ]
